...
export default function StudentDirectory() {
  ...
  // Derived Data
  const class11Count = students.filter(s => s.class === '11').length
  const class12Count = students.filter(s => s.class === '12').length

  const classStudents = students.filter(s => s.class === selectedClass)
  const sectionStudents = classStudents.filter(s => s.section === selectedSection)
  ...
  const stats = {
    ...
    active: sectionStudents.filter(s => s.status === 'active' || !s.status).length, // assuming default is active
    inactive: sectionStudents.filter(s => s.status === 'inactive').length,
    pending: sectionStudents.filter(s => s.status === 'pending').length
  }
        ...
        <div>
          {selectedClass ? (
            <div className="flex items-center gap-3 text-slate-400 font-bold text-xs uppercase tracking-widest mb-4">
              ...
              <span className="text-slate-800">Class {selectedClass}</span>
            </div>
          ) : null}
          <h1 className="text-4xl font-black text-slate-800 tracking-tight mb-2">
            {selectedClass ? `Class ${selectedClass} Directory` : 'Student Directory'}
          </h1>
          <p className="text-slate-500 font-medium text-lg">
            {selectedClass ? `Managing ${classStudents.length} students in Class ${selectedClass}` : `${students.length} students currently enrolled`}
          </p>
        </div>
        <button 
      ...
      {!selectedClass ? (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <StatCard label="Total Students" value={stats.total} icon={Users} color="bg-indigo-500" />
            <StatCard label="Active Students" value={stats.active} icon={UserCheck} color="bg-emerald-500" />
            <StatCard label="Inactive" value={stats.inactive} icon={UserX} color="bg-rose-500" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    {['Avatar', 'Student ID', 'Full Name', 'Reg. No', 'Roll No', 'Status', 'Action'].map(h => (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredStudents.map((s) => (
                    <tr key={s._id} className="group hover:bg-slate-50/30 transition-all">
                      ...
                      <td className="px-8 py-6 text-slate-500 font-medium text-sm">{s.rollNumber || '—'}</td>
                      <td className="px-8 py-6">
                        <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                          s.status === 'inactive' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'
                        }`}>
                          {s.status || 'Active'}
                        </span>
                      </td>
                  ...
                  {filteredStudents.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-8 py-32 text-center text-slate-300">
                        ...
                        <p className="font-black uppercase tracking-widest text-xs">No student records found</p>
                      </td>
                    </tr>
                  )}
                </tbody>
